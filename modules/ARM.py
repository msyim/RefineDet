...
class ARM(nn.Module):
    ...
    def forward(self, conv4_3, conv5_3):
        ...
        conv_fc7 = self.conv_fc7(self.conv_fc6(self.mp(conv5_3)))

        # The following 4 feature maps will be used in training
        # 1. L2Normed conv4_3
        # 2. L2Normed conv5_3
        # 3. conv6_1 (one of the two extra layers to capture high-level information)
        # 4. conv6_2 (one of the two extra layers to capture high-level information)
        l2_conv4_3 = self.conv4_l2norm(conv4_3)
        l2_conv5_3 = self.conv5_l2norm(conv5_3)
        conv6_1    = self.conv6_1(conv_fc7)
        conv6_2    = self.conv6_2(conv6_1)

        # Apply ARM loc
        conv4_3_loc = self.conv4_3_loc(l2_conv4_3)
        conv5_3_loc = self.conv5_3_loc(l2_conv5_3)
        conv6_1_loc = self.conv6_1_loc(conv6_1)
        conv6_2_loc = self.conv6_2_loc(conv6_2)

        # Apply ARM conf
        conv4_3_conf = self.conv4_3_conf(l2_conv4_3)
        conv5_3_conf = self.conv5_3_conf(l2_conv5_3)
        conv6_1_conf = self.conv6_1_conf(conv6_1)
        conv6_2_conf = self.conv6_2_conf(conv6_2)

        featmap_list = [l2_conv4_3, l2_conv5_3, conv6_1, conv6_2]
        armloc_list  = [conv4_3_loc, conv5_3_loc, conv6_1_loc, conv6_2_loc]
        armconf_list = [conv4_3_conf, conv5_3_conf, conv6_1_conf, conv6_2_conf]
